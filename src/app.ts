...
type Tone = "warm" | "direct" | "celebratory";
...
const defaultEmails: OnboardingEmail[] = [
  {
    day: 1,
...
function isEmail(value: unknown): value is OnboardingEmail {
...
function normalizeEmails(value: unknown, fallback: OnboardingEmail[]): OnboardingEmail[] {
...
export function selectDay(state: AppState, day: number): AppState {
...
export function updateEmail(state: AppState, day: number, patch: EmailPatch): AppState {
  return {
    ...state,
...
export function resetFlow(state: AppState): AppState {
...
export function formatEmail(email: OnboardingEmail): string {
  return [
    `Day ${email.day}`,
...
export function exportFlow(state: AppState): string {
...
function initializeApp() {
  ...
  function selectedEmail(): OnboardingEmail {
  ...
  function renderPreview(email: OnboardingEmail) {
  ...
  function patchSelectedEmail(patch: EmailPatch) {
  ...
  elements.toneSelect.addEventListener("change", () => {
    patchSelectedEmail({ tone: elements.toneSelect.value as Tone });
  });
